...
- App Server 返回使用额度耗尽：本轮不计数，保持 `waiting`，优先等到错误中给出的额度重置时间后重试；没有可解析时间时使用最长 1 小时的退避。
...
- App Server usage limit: the attempt is not counted, the loop remains `waiting`, and retries after the reported reset time when available; otherwise it uses backoff capped at one hour.
