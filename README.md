...
兼容模式由同步 Stop Hook 等待；`Ctrl-C` 可终止等待。推荐模式下，`loopcodex` 函数为每个 TUI 创建独立 App Server，并在会话退出时清理。Loop 会保留一个休眠中的一次性 Node 唤醒进程，每次唤醒后该进程退出。没有操作系统 Cron 或本项目自己的常驻 daemon。电脑、App Server 与当前会话必须保持运行。
