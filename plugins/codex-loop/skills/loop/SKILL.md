...
Codex Loop is the sole scheduler and lifetime owner for requests handled by this skill. A request to loop, repeat, keep checking, or run for a duration is not a request for a Codex durable goal. Never call `create_goal` for the same work, and never combine goal auto-continuation with a Codex Loop. The two mechanisms both own turn continuation and will prevent the Stop hook from observing a completed pass.
...
3. Before performing project setup, validation, or task work, run one of these forms. The first Loop pass owns that work:
...
- Do not create, inspect, update, or depend on a durable goal while preparing the Loop. If the current turn is already an automatic durable-goal continuation, do not start a competing Loop in that turn.
...
- Do not call `create_goal` or use goal auto-continuation. End the turn after the single pass so the Stop hook can record it and schedule the next pass.
